...
class GoldRush(Matrix):
    ...
    def load_board(self):
        ...
        for i in range(self.rows):
            ...
            for k in range(1, self.cols, rand):
                ...
                rand_index = random.randint(0, 1)
                rand_element = elements[rand_index]
                current = self.matrix[i][k]
                self.matrix[i][k] = rand_element
                if rand_element == "coin" and current != "coin":
                    coins += 1
        ...
        if coins < MIN_ALLOWED_COINS:
            return self.load_board()
        else:
            return self.matrix

    def _check_if_player_has_won(self, player):
        player_num = player[-1]
    ...
    def _move(self, curr_row, curr_col, player, delta_row, delta_col):
        ...
        return self._check_if_player_has_won(player)
